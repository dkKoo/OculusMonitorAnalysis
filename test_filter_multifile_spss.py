...
def save_stats(stats_list, output_dir):
    for i, stats in enumerate(stats_list):
        file_name = f'stats_{i+1}.csv'
        file_path = os.path.join(output_dir, file_name)
        
        with open(file_path, 'w', newline='') as file:
            csv_writer = csv.writer(file)
            csv_writer.writerow(['Variable', 'Max', 'Min', 'Mean', 'Std', 'CV', 'Integral'])
            for key, value in stats.items():
                if key != 'Correlation':
                    csv_writer.writerow([key, value['max'], value['min'], value['mean'], value['std'], value['cv'], value.get('integral', '')])
            
            csv_writer.writerow([])
            csv_writer.writerow(['Correlation', 'Value'])
            for corr_key, corr_value in stats['Correlation'].items():
                csv_writer.writerow([corr_key, corr_value])
...
def save_stats(stats_list):
    # stats_list를 데이터프레임으로 변환
    df = pd.DataFrame(stats_list)
    return df

def save_spss_data(df, output_dir):
    # 데이터프레임 재구성
    data = []
    for idx, stats in enumerate(df['HeadPosX']):
        row = {'SubjectID': idx + 1}
        for key in ['HeadPosX', 'HeadPosY', 'HeadPosZ', 'HeadOrientationW', 'HeadOrientationX', 'HeadOrientationY', 'HeadOrientationZ', 'VelocityX', 'VelocityY', 'VelocityZ', 'Speed']:
            for subkey in ['max', 'min', 'mean', 'std', 'cv', 'integral']:
                if subkey in df[key][idx]:
                    row[f"{key}_{subkey}"] = df[key][idx][subkey]
        for corr_key in ['HeadPosXY', 'HeadPosXZ', 'HeadPosYZ', 'VelocityXY', 'VelocityXZ', 'VelocityYZ']:
            row[corr_key] = df['Correlation'][idx][corr_key]
        data.append(row)
    
    spss_df = pd.DataFrame(data)
    
    # 결측값 처리
    spss_df = spss_df.fillna(-999)
    
    # 파일 경로 설정
    file_name = 'spss_data.csv'
    file_path = os.path.join(output_dir, file_name)
    
    # 데이터프레임을 CSV 파일로 저장
    spss_df.to_csv(file_path, index=False)
    
    print(f"SPSS 분석을 위한 데이터가 {file_path}에 저장되었습니다.")
...
if file_paths and output_dir:
    ...
    # stats_list를 데이터프레임으로 저장
    df = save_stats(stats_list)
    
    # SPSS 분석을 위한 데이터 저장
    save_spss_data(df, output_dir)
